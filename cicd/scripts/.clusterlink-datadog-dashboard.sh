...
kubectl get secret kafka-secrets -n confluent || kubectl create secret generic kafka-secrets --from-file=keystore.p12=keystore.p12 --from-file=truststore.p12=truststore.p12 -n confluent 
...
# EOF
kubectl get configmap kafka-client-config -n confluent || kubectl create configmap kafka-client-config --from-file=client.properties -n confluent